...
function useCountUp(max: number, ms: number, on = true) {
...
/** fade in */
const FADE = "transition-opacity duration-500 motion-reduce:transition-none starting:opacity-0";
...
/** Rafi on the other end of the line, as a chat bubble under the stage. */
function Rafi({ tone = "plain", children }: { tone?: "plain" | "good" | "bad"; children: ReactNode }) {
  return (
    <div className="mt-3 flex items-start gap-2.5 transition duration-300 motion-reduce:transition-none starting:translate-y-1 starting:opacity-0">
      <span
        aria-hidden="true"
        className="grid size-8 shrink-0 place-items-center rounded-full bg-cat-blue/15 text-sm font-bold text-cat-blue"
      >
        র
      </span>
      <div
        className={`rounded-2xl rounded-tl-sm px-3.5 py-2 text-[0.95rem] leading-snug ${
          tone === "good" ? "bg-accent/10 text-accent-text" : tone === "bad" ? "bg-danger/10 text-danger" : "bg-cat-blue/10"
        }`}
      >
        <b className="font-semibold">রাফি:</b> {children}
      </div>
    </div>
  );
}
...
type Look = "idle" | "picked" | "right" | "wrong" | "dim";
...
const predictLook = (i: number, guess: number | null, over: boolean, answer: number): Look =>
...
function Ticks({ items }: { items: [label: string, done: boolean][] }) {
  return (
